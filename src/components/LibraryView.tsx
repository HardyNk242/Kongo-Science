...
const LibraryView: React.FC = () => {
  ...
  // @ts-ignore (Si TypeScript râle sur le map, ignorez-le pour l'instant)
  const domains = ['Tous', ...new Set(THESES_LIBRARY.map((t: Thesis) => t.domain))];

  const filteredTheses = THESES_LIBRARY.filter((t: Thesis) => {
  ...
  return (
    <div className="bg-white min-h-screen">
      {/* Hero Section Académique */}
      <section className="bg-slate-900 pt-32 pb-24 text-white relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          <span className="text-blue-400 font-black text-[10px] uppercase tracking-[0.4em] mb-4 block">Archive Scientifique Souveraine</span>
          <h1 className="text-4xl md:text-6xl font-serif font-bold italic mb-6">Bibliothèque des Publications</h1>
      ...
      <section className="sticky top-20 z-40 bg-white/80 backdrop-blur-md border-b border-slate-100 py-4 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 flex items-center gap-4 overflow-x-auto pb-2 md:pb-0">
          <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest whitespace-nowrap">Disciplines :</span>
          {domains.map((domain: unknown) => (
            <button
              key={domain as string}
              onClick={() => setSelectedDomain(domain as string)}
              ...
              <div key={thesis.id} className="group bg-white rounded-[2.5rem] border border-slate-100 p-8 md:p-12 hover:shadow-2xl hover:border-blue-100 transition-all duration-500 flex flex-col md:flex-row gap-10 items-start">
                ...
                <div className="flex-grow">
                   ...
                   <div className="flex flex-wrap gap-4 pt-6 border-t border-slate-50">
                      ...
                      <button className="bg-blue-700 text-white px-8 py-3 rounded-xl font-bold text-sm shadow-xl shadow-blue-100 hover:bg-blue-800 hover:-translate-y-1 transition-all ml-auto flex items-center gap-2">
      ...
      {/* Bandeau de contribution */}
      <section className="bg-blue-50 py-24">
         <div className="max-w-4xl mx-auto px-6 text-center">
            <h2 className="text-3xl font-serif font-bold text-slate-900 italic mb-6">Votre recherche mérite d'être lue.</h2>
            <p className="text-slate-600 text-lg mb-10">
               Vous êtes un chercheur ou venez de valider votre Master avec brio ? <br/> Ne laissez pas vos découvertes dans l'ombre.
            </p>
            <button className="bg-slate-900 text-white px-10 py-5 rounded-2xl font-black text-lg hover:bg-blue-700 transition-all shadow-2xl">
               Soumettre ma publication
            </button>
            <p className="mt-8 text-xs text-slate-400 uppercase tracking-widest font-bold">Processus de vérification scientifique rigoureux</p>
         </div>
      </section>
    </div>
  );
