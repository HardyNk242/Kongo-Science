...
const LibraryView: React.FC = () => {
  ...
  const renderZoteroCoins = (thesis: Thesis) => {
    // ... (Code Zotero identique)
    return <span className="Z3988" style={{ display: 'none' }}></span>;
  };
      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full relative animate-fadeIn">
          ...
          <div className="text-center mb-6">
             ...
             <p className="text-sm text-slate-500 mt-2">Ce document est protégé par droits d'auteur.</p>
          </div>
          <div className="space-y-3">
            ...
            <button onClick={() => { navigator.clipboard.writeText("nkodiahardy@gmail.com"); alert("Email copié !"); }} className="flex items-center justify-center gap-3 w-full bg-slate-100 text-slate-700 hover:bg-slate-200 p-3 rounded-xl font-bold transition-colors">
              Copier l'email
            </button>
            <a href={mailtoLink} className="block text-center text-xs text-slate-400 hover:text-blue-600 underline mt-4">Ou ouvrir l'app par défaut</a>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="bg-white min-h-screen pt-32 pb-20 px-6">
        ...
        <div className="max-w-4xl mx-auto">
          <button onClick={() => setSelectedThesis(null)} className="group flex items-center gap-2 text-sm text-slate-500 hover:text-blue-700 mb-8 transition-colors">
            <svg className="w-4 h-4 group-hover:-translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg> Retour aux résultats
          </button>
          ...
          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl p-8 md:p-12">
             <div className="flex flex-col md:flex-row gap-6 mb-8 border-b border-slate-100 pb-8">
                ...
                <div>
                   <div className="flex flex-wrap gap-2 mb-3">
                     <span className="inline-block bg-blue-50 text-blue-700 text-xs font-bold px-3 py-1 rounded-full border border-blue-100">{selectedThesis.type}</span>
                     {selectedThesis.isForSale && (
             ...
             <div className="flex flex-col sm:flex-row gap-4 border-t border-slate-50 pt-8">
                ...
                <button onClick={() => copyCitation(selectedThesis)} className="bg-slate-50 hover:bg-slate-100 text-slate-700 px-6 py-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2 border border-slate-200 w-full sm:w-auto">
                   Citer
                </button>
             </div>
          </div>
    ...
    <div className="bg-white min-h-screen">
      ...
      <section className="bg-slate-900 pt-32 pb-12 text-white sticky top-0 z-40 shadow-xl">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row gap-6 items-center">
             <div className="relative flex-grow w-full">
               <input type="text" placeholder="Rechercher..." className="w-full h-12 pl-12 pr-4 rounded-lg text-slate-900 bg-white shadow-inner focus:ring-2 focus:ring-blue-500 outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
             ...
             <button onClick={() => setShowSubmitModal(true)} className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-lg font-bold whitespace-nowrap transition-colors flex items-center gap-2">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg> Soumettre
             </button>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 flex flex-col md:flex-row gap-12">
        <aside className="w-full md:w-64 flex-shrink-0 space-y-8">
           <div>
             <h4 className="text-sm font-bold text-slate-900 uppercase tracking-widest mb-4 border-b border-slate-200 pb-2">Année</h4>
             <div className="flex flex-col gap-2">
               {[{ label: 'N\'importe quand', val: 'any' }, { label: 'Depuis 2026', val: '2026' }, { label: 'Depuis 2025', val: '2025' }, { label: 'Depuis 2022', val: '2022' }].map((opt) => (
                 <button key={opt.val} onClick={() => setSelectedYearFilter(opt.val)} className={`text-left text-sm transition-colors ${selectedYearFilter === opt.val ? 'font-bold text-blue-700' : 'text-slate-600 hover:text-blue-600 hover:underline'}`}>{opt.label}</button>
               ))}
             </div>
           </div>
           <div>
             <h4 className="text-sm font-bold text-slate-900 uppercase tracking-widest mb-4 border-b border-slate-200 pb-2">Domaines</h4>
             <div className="flex flex-col gap-2">
               {domains.map((domain) => (
                 <button key={domain} onClick={() => setSelectedDomain(domain)} className={`text-left text-sm transition-colors ${selectedDomain === domain ? 'font-bold text-blue-700' : 'text-slate-600 hover:text-blue-600 hover:underline'}`}>{domain}</button>
               ))}
             </div>
           </div>
        </aside>
        ...
        <main className="flex-grow">
            <div className="mb-6 text-slate-400 text-sm">Environ {filteredTheses.length} résultats</div>
            <div className="space-y-8">
              {currentTheses.length > 0 ? (
                currentTheses.map((thesis) => (
                  <div key={thesis.id} className="group relative">
                    <h3 onClick={() => setSelectedThesis(thesis)} className="text-xl font-serif text-blue-800 font-medium cursor-pointer hover:underline hover:text-blue-600 mb-1 leading-snug">{thesis.title}</h3>
                    <div className="text-sm text-green-800 mb-2 font-medium">{thesis.author} - <span className="italic">{thesis.institution}</span>, {thesis.year}</div>
                    <p className="text-slate-600 text-sm leading-relaxed line-clamp-2 mb-2 max-w-3xl">{thesis.abstract}</p>
                    <div className="flex flex-wrap items-center gap-4 text-xs">
                       ...
                       <button onClick={() => copyCitation(thesis)} className="text-slate-400 hover:text-blue-700 hover:underline">Citer</button>
                ...
                <div className="py-20 text-center bg-slate-50 rounded-xl border border-slate-100">
                  ...
                  <button onClick={() => {setSearchTerm(''); setSelectedDomain('Tous'); setSelectedYearFilter('any');}} className="mt-4 text-blue-600 font-bold hover:underline">Effacer les filtres</button>
                </div>
              )}
            </div>
            ...
            {totalPages > 1 && (
              <div className="mt-16 pt-8 border-t border-slate-100 flex justify-center items-center gap-2 select-none">
                <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="px-3 py-2 text-sm text-blue-700 hover:underline disabled:opacity-30 disabled:no-underline">Précédent</button>
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                  <button key={page} onClick={() => setCurrentPage(page)} className={`w-8 h-8 rounded text-sm font-bold transition-all ${currentPage === page ? 'text-slate-900' : 'text-blue-700 hover:underline'}`}>{page}</button>
                ))}
                <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="px-3 py-2 text-sm text-blue-700 hover:underline disabled:opacity-30 disabled:no-underline">Suivant</button>
              </div>
            )}
        </main>
      </div>
