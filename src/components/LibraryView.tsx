...
const LibraryView: React.FC = () => {
  ...
  // --- 1. CALCUL DES COMPTEURS PAR DOMAINE (NOUVEAU) ---
  const domainCounts = useMemo(() => {
    // On initialise un compteur vide
    const counts: Record<string, number> = {};
    
    // On parcourt toute la bibliothèque pour compter
    THESES_LIBRARY.forEach((item) => {
      // On normalise le domaine (parfois undefined ou null dans les imports massifs)
  ...
  // --- ALGORITHME PAGINATION "GOOGLE STYLE" (NOUVEAU) ---
  const getVisiblePages = () => {
    ...
    range.push(1); // Toujours afficher la page 1
    ...
    range.push(totalPages); // Toujours afficher la dernière page
           ...
           <div>
             ...
             <div className="flex flex-col gap-2 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
               ...
               {SCIMAGO_DOMAINS.map((dom) => {
                 ...
                 if (count === 0) return null; // Masquer les catégories vides ? Optionnel.
                  ...
                  typeof page === 'number' ? (
                    <button
                      ...
                      className={`w-8 h-8 rounded text-sm font-bold transition-all ${
                        currentPage === page
                        ? 'bg-blue-600 text-white shadow-md' // Style actif plus visible
